        ...
        <Container>
          ...
          >
            ...
            <div className="space-y-5 text-base leading-8 text-[var(--rito-muted)]">
              {hierarquiaPage.introducao.map((paragrafo) => (
      ...
      <Section>
        <Container>
          <div className="grid gap-8">
            {hierarquiaCargos.map((cargo, index) => (
              <motion.article
                key={cargo.titulo}
                initial={{ opacity: 0, y: 26 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                  <div className="rounded-2xl border border-[var(--rito-border)] bg-black/20 p-5">
                    ...
                    <ul className="space-y-3 text-sm leading-7 text-[var(--rito-muted)]">
                      {cargo.responsabilidades.map((item) => (
                        <li key={item}>✦ {item}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="rounded-2xl border border-[var(--rito-border)] bg-black/20 p-5">
                    ...
                    <div className="space-y-3 text-sm leading-7 text-[var(--rito-muted)]">
                      {cargo.conquista.map((item) => (
                        <p key={item}>{item}</p>
                      ))}
                    </div>
                  </div>
      ...
      <Section>
        <Container>
          ...
          >
            ...
            <h2 className="font-arcane text-4xl text-[var(--rito-ivory)] md:text-6xl">
              {evolucaoHierarquia.titulo}
            </h2>

            <Divider />

            <div className="space-y-5 text-base leading-8 text-[var(--rito-muted)]">
              {evolucaoHierarquia.texto.map((paragrafo) => (
                <p key={paragrafo}>{paragrafo}</p>
              ))}
            </div>
          </motion.article>
